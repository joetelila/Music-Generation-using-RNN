...
% tt iif L1 and L2 has a common element
common_member(L1,L2) :-
	member(E,L1),
	member(E,L2).

% reads an external KB which contains name of all countries in the world
% and convert it into a string list
read_KB(S) :-
    csv_read_file("country.csv", Rows, [separator(0';)]),
    maplist(arg(1), Rows, Names),
    maplist(atom_string, Names, S).

% predicate to start the chat
chat :- 
...
% quit and "Tell me more" predicates
recognize(L,99) :- member("quit",L);member("exit",L).
recognize(_,100). 

% saves emotions into file
saveinto(L) :- 
	not(member("angry",L)),
	not(member("happy",L)),
	not(member("sad",L)),
	not(member("surprise",L)),
	not(member("fear",L)),
	not(member("disgust",L)),
	true.
	
saveinto(L) :-
	open('emotions.txt', append, Out),
	member("sad",L),
	write(Out, "sad"),
	nl(Out),
	close(Out).
